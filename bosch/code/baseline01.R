...
# see forums for additional discussion
...
###################################################
##### sample and aggregate at the Station level ###
###################################################
...
getstations <- function(chunksize, s){
  ...
  cats <- fread("../input/train_categorical.csv", colClasses = catclassvector, na.strings=""
  ...
  nums <- fread("../input/train_numeric.csv", colClasses = numclassvector, na.strings=""
                , stringsAsFactors=FALSE, nrows = chunksize, skip=s)
