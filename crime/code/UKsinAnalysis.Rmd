...
crime.raw.clean.type.outcome <- crime.raw.clean %>%
  group_by(Crimetype,Lastoutcomecategory ) %>%
  summarise(num_crimes = n())

ggplot(crime.raw.clean.type.outcome, aes(x = Crimetype, y = num_crimes, fill = Lastoutcomecategory)) +
  geom_bar(stat = 'identity', position = 'fill', colour = 'black') +
  coord_flip() +
  labs(y = 'Proportion of crimes', 
       x = 'Crimes',
       title = 'Outcomes') +  theme_few()

crime.raw.void.type.outcome <- crime.raw.void %>%
  group_by(Crimetype,Lastoutcomecategory ) %>%
  summarise(num_crimes = n())


ggplot(crime.raw.void.type.outcome, aes(x = Crimetype, y = num_crimes, fill = Lastoutcomecategory)) +
  geom_bar(stat = 'identity', position = 'fill', colour = 'black') +
  coord_flip() +
  labs(y = 'Proportion of crimes', 
       x = 'Crimes',
...
```{r outcomes file, tidy=T}


crime.outcomes <-fread( '../input/outcomes20122015.csv' , 
           
            na.strings=missing.types, 
          verbose = FALSE, showProgress = TRUE )%>% as.data.frame() 

crime.outcomes <- readData('../input/outcomes20122015.csv',   missing.types, types  )
names(crime.outcomes)

row.has.na <- apply(crime.outcomes, 1, function(x){any(is.na(x))})

crime.outcome.void <- crime.outcomes[row.has.na,]

crime.outcome.clean <- crime.outcomes[!row.has.na,]
```
