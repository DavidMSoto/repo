...
#tomate un tiempo para hacerte unas cuantas pajas mentales sobre los datos
#se pueden inferir las variables que faltan ..
# digo yo que el piso en el que estaban es importante .. pero .. como inferir esa variable si casi todas estan vacias, 
# podemos inferirla sobre el precio del ticket ? sobre si son familia o no ? 
...
df.train$FamilySizeAdj <- df.train$FamilySize
print(table(df.train$FamilySize))

sharedInd <- df.train$FamilySizeAdj == 1 & df.train$Cabin %in% sharedCabins
df.train$FamilySizeAdj[sharedInd] <- 2
rowCount <- sum(sharedInd)
print(c("adjusted rows", rowCount)) # 27 rows
print(table(df.train$FamilySizeAdj))
